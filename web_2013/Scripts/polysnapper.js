...
/*KJA added functions to here so in one place*/
function LoadSMPolygons(parms) {
  var arg;
  SMPolygons = [];
  for (var argIx = 0; argIx < arguments.length; argIx++) {
    arg = arguments[argIx];
    var feat;
    for (var featIx = 0; featIx < arg.features.length; featIx++) {
      feat = arg.features[featIx];
      SMPolygons.push(feat.geometry);
    }
  }
  //console.log(SMPolygons);
}
function SetSM(polystyle, isPoly) {
  //from docs
  SM = PolySnapper({
    map: gglmap,
    ...
    isPoly: isPoly,
    hidePOI: false,
    onEnabled: function () { /*console.log("enabled");*/ },
    onDisabled: function () { /*console.log("disabled");*/ }
  });
}
/*end KJA added*/
...
function PolySnapper(opts) {
  ...
  function defined(obj, key) { return typeof obj[key] !== 'undefined'; }
  ...
  var _isPoly = (defined(opts, 'isPoly')) ? opts.isPoly : true;
  var _hidePOI = (defined(opts, 'hidePOI')) ? opts.hidePOI : false;

  var _keyDown = false;

  if (!_map) { console.log("We need to know the map"); return; }
  ...
  return {
    ...
    enable: function (poly) {
      that.drawing = true;
      if (_hidePOI) _map.poi(false);

      var vertexMarker = _marker;
      var snapable_polys = that.polys; //ALL! .filter(function (p) { return (typeof p.snapable !== 'undefined' && p.snapable) });
      ...
      if (poly) {
        ...
        if (_isPoly) {
          ...
          that.currentpoly = new google.maps.Polyline(
            extend(_polystyle, { editable: true, map: _map })
          );
        }
      }
    ...
    disable: function () {
      ...
      if (_marker) { _marker.setMap(null); _marker = null; } //added KJA
      //annnd the callback
      _onDisabled();
    }
  }
